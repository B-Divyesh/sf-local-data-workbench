...
async function mockRelease(page: Page, body: object = {
  tag_name: 'v0.1.5', target_commitish: 'candidate-commit', body: 'Build commit: candidate-commit\nmacOS signing: unavailable\nWindows signing: unavailable', assets: [
    { name: 'Local_Data_Workbench_0.1.5_amd64.AppImage', browser_download_url: 'https://github.com/B-Divyesh/sf-local-data-workbench/releases/download/v0.1.5/Local_Data_Workbench_0.1.5_amd64.AppImage' }
  ]
}): Promise<void> {
...
test('@claim:build-identity built pages identify their version and source revision', async ({ page }) => {
  ...
  await expect(page.locator('#build-id')).toContainText('0.1.5');
});
...
test('@claim:signed-platform-gating unsigned release assets never enable macOS or Windows downloads', async ({ page }) => {
  await mockRelease(page, {
    tag_name: 'v0.1.5', target_commitish: 'candidate-commit', body: 'Build commit: candidate-commit\nmacOS signing: unavailable\nWindows signing: unavailable', assets: [
      { name: 'Local_Data_Workbench_0.1.5_x64.dmg', browser_download_url: 'https://example.invalid/unsigned.dmg' },
      { name: 'Local_Data_Workbench_0.1.5_x64-setup.exe', browser_download_url: 'https://example.invalid/unsigned.exe' },
      { name: 'Local_Data_Workbench_0.1.5_amd64.AppImage', browser_download_url: 'https://example.invalid/linux.AppImage' }
    ]
  });
  await page.goto('http://127.0.0.1:4173/');
